...
int UpdateGame(void) {
  ...
  if (updateGameIterations == target_fps * STACK_PRINT_TIME_S_MOD) {
    ...
    for (size_t i = 0; i < stack->top; i++) {
      LogMessage(LOG_INFO, "\tStack Index: {%zu} | State {%s}", i,
                 StateToString(stack->states[i]));
    }
  ...
  case GAME_STATE_PLAYING:
    ...
    // CheckPlayerChunkCollision(p_player, p_chunks, chunkCount); // Recolors
    // player UpdateChunks(); UpdateParallaxBackground(p_player->velocity.x);
